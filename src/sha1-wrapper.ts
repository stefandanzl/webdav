...
(function() {
    ...
    const NODE_JS = !root.JS_SHA1_NO_NODE_JS && typeof process === 'object' && process.versions && process.versions.node;
    if (NODE_JS) {
      root = global;
    }
    ...
    let nodeWrap = function (method) {
      let crypto = eval("require('crypto')");
      let Buffer = eval("require('buffer').Buffer");
